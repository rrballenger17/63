...
data = records.map(lambda r: LabeledPoint(extract_label_acc(r),extract_features(r)))

# get 90% train and 10% test data
data_with_idx = data.zipWithIndex().map(lambda (k, v): (v, k))
test = data_with_idx.sample(False, 0.13, 100)
...
# make the acceleration linear model
linear_model_acc = LinearRegressionWithSGD.train(train_data, iterations=100, step=0.0000001, intercept=False)
linear_model_acc

# predict test values and measure error 
true_vs_predicted = test_data.map(lambda p: (p.label, linear_model_acc.predict(p.features)))
